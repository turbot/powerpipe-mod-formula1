...
query "total_races" {
  sql = <<-EOQ
    select count(*) as "Total Races" from races;
  EOQ
}

query "total_drivers" {
  sql = <<-EOQ
    select count(distinct driverId) as "Total Drivers" from drivers;
  EOQ
}

query "total_constructors" {
  sql = <<-EOQ
    select count(distinct constructorId) as "Total Constructors" from constructors;
  EOQ
}

query "total_seasons_count" {
  sql = <<-EOQ
    select count(*) as "Total Seasons" from seasons;
  EOQ
}
...
query "team_standings_by_conference" {
  sql = <<-EOQ
    ...
    group by constructor_standings.constructorId
    order by "wins" desc;
  EOQ
}
...
query "average_attendance_per_game" {
  sql = <<-EOQ
    ...
    from (
      select r.date, count(*) as total_results
      from races r
